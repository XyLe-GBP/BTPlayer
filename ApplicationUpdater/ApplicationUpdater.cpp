...
int main()
{
	...
	if (0 != GetModuleFileName(NULL, Path, MAX_PATH)) {
		...
		HWND handle = FindWindow(_T("btplayer"), NULL);
		...
		if (PathFileExists(common->StringToWString(sdrive + sdir + "updater.dat").c_str())) {
			...
			if (PathFileExists(common->StringToWString(sdrive + sdir + "btplayer.zip").c_str())) {
				HRESULT hr;
				...
				TCHAR* SourcePath = common->ConvertTCHAR(common->StringToWString(sdrive + sdir + "btplayer.zip").c_str());
				...
				if (common->ExtractZip(pShellDisp, SourcePath, ExtPath) == TRUE) {
					cout << "'" + sdrive + sdir + "btplayer.zip' to '" + sdrive + sdir + "updater-temp' extracted." << endl;
					CoUninitialize();
					...
					cout << "\033[31m" << "ERROR: '" + sdrive + sdir + "btplayer.zip' to '" + sdrive + sdir + "updater-temp' extract failed." << "\033[m" << endl;
					CoUninitialize();
					SAFE_FREE(SourcePath);
					SAFE_FREE(ExtPath);
					SAFE_DELETE(common);
				...
				wstring ExectablePath = common->StringToWString(AppPath) + L"\\btplayer.exe";
